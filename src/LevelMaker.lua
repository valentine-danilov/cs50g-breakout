...
local function generateLockedBricks(remainingNumberOfBlockedBricks, numRows, numCols)
    local doGenerateLockedBricks = math.random(numRows)
    local lockedBricks = {}
    if not doGenerateLockedBricks then return lockedBricks end

    local numberOfLockedBricksInARow = math.min(math.min(remainingNumberOfBlockedBricks, LOCKED_BRICKS_MAX_PER_ROW),
        math.random(LOCKED_BRICKS_MAX_PER_ROW))

    if numCols % 2 == 0 and numberOfLockedBricksInARow == 3 then
        numberOfLockedBricksInARow = 2
    end

    if (numberOfLockedBricksInARow == 1) then
        table.insert(lockedBricks, {
            ['x'] = math.random(numCols)
        })
    end

    if (numberOfLockedBricksInARow == 2) then
        local first = math.random(numCols / 2)
        local second = numCols - first + 1
        table.insert(lockedBricks, {
            ['x'] = first,
            ['included'] = false
        })
    ...
    if (numberOfLockedBricksInARow == 3) then
        local middle = (numCols / 2) + 1
        local left = math.random(numCols / 2)
        local right = numCols - left - 1
        table.insert(lockedBricks, {
            ['x'] = left,
            ['included'] = false
        })
...
function LevelMaker.createMap(level)
    local numberOfLockedBricks = level / 10 + 1
    local bricks = {}
    ...
    for y = 1, numRows do
        ...
        local lockedBricks = generateLockedBricks(numberOfLockedBricks, numRows, numCols)
        numberOfLockedBricks = math.max(0, numberOfLockedBricks - #lockedBricks)
        ...
        for x = 1, numCols do
            ...
            -- x-coordinate
                ...
                * 32 -- multiply by 32, the brick width
                ...
                -- y-coordinate
                y * 16 + 4 -- just use y * 16, since we need top padding anyway
            )

            for _, lockedBrick in pairs(lockedBricks) do
                if x == lockedBrick['x'] and not lockedBrick['included'] then
                    b.locked = true
                    lockedBrick.included = true
