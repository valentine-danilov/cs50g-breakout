...
function love.load()
    ...
    gFrames = {
        ...
        ['locked-brick'] = LockedBrickQuad(gTextures['main'])
    }
...
function renderActivePowerups(powerups)
    local powerupsX = VIRTUAL_WIDTH - 120

    for _, powerup in pairs(powerups) do
        if powerup.active then
            love.graphics.draw(gTextures['main'], gFrames['powerups'][powerup.type], powerupsX, 2)
            powerupsX = powerupsX - 18
        end
    end
end
